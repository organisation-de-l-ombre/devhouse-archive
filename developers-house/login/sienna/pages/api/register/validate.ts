  ...
  if (_csrf && validate && csrfKey && check(csrfKey, _csrf) && register) {
    ...
    if (validate === "accept") {
      ...
      const resp = await fetch(
        `${process.env.ELLIE_ENDPOINT}/avatar-link?link=${encodeURIComponent(
      ...
      if (resp.ok) {
        ...
        const result = await fetch(
          `${process.env.SCARLET_ENDPOINT}/api/users/create`,
          {
        ...
        if (result.ok) {
          ...
          const data = await AdminAPI.acceptLoginRequest(
            register.loginChallenge,
            {
              subject: user.uuid,
