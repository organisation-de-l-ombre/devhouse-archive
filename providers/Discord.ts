...
export default class DiscordProvider implements Provider {
    ...
    getRedirectUri(state: string, host: string): string {
        return `https://discord.com/api/oauth2/authorize?client_id=${this.props.client_id}&scope=identify&redirect_uri=${encodeURIComponent(`https://${host}${this.props.redirect_uri}`)}&state=${encodeURIComponent(state)}&response_type=code&prompt=none`;
    }
